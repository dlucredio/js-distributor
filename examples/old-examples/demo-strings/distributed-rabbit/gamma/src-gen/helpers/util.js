...
async function toLowerCaseString(str) {
    const p = new Promise(async (resolve, reject) => {
        try {
            ...
            console.log("Connection established!");
            console.log("Sending call to function toLowerCaseString");
            const channel = await connection.createChannel();
            ...
            const queueName = "toLowerCaseString_queue";
            ...
            channel.consume(q.queue, (msg) => {
                if (msg) {
                    const message = JSON.parse(msg.content.toString());
                    console.log("Receiving response for function toLowerCaseString");
                    if (msg.properties.correlationId === correlationId) {
                        const result = message.result;
            ...
            console.log("Sending message to queue: toLowerCaseString_queue");
            channel.sendToQueue(queueName, Buffer.from(JSON.stringify(callObj)), {
                correlationId: correlationId,
                replyTo: q.queue
            });
        } catch (error) {
            console.error("Error processing call to function toLowerCaseString:", error);
            reject(error);
        }
    });
    return p;
